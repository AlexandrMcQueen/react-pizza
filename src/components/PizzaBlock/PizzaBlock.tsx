import React, {useState} from 'react';
import {useDispatch, useSelector} from "react-redux";
import {addProduct, selectCartItemById} from "../../redux/slices/cartSlice";
import {Link} from "react-router-dom";

type PizzaBlockProps = {
  title: string,
  price: number,
  imageUrl: string,
  sizes : number[],
  types: number[],
  id: string,

}
type count =  number;
export const PizzaBlock : React.FC<PizzaBlockProps> = ({title,price,imageUrl,sizes,types,id}) => {
    const [activeType,setActiveType] = useState(0);
    const [sizeActive,setSizeActive] = useState(0);
    const typeNames = ['тонке',"традиційне"];
    const activeSizeValue = [26,30,40];


    const dispatch = useDispatch();
    const cartItem = useSelector(selectCartItemById(id));

    const addedCount = cartItem ? cartItem.count : 0;

        const onClickAdd = () => {
        dispatch(addProduct( {count:0,title,price,imageUrl,id,type:typeNames[activeType],size:activeSizeValue[sizeActive]}))
      }



    return (
                <div key={id} className="pizza-block" >
                <Link to = {`/pizza/${id}`} >
                    <img
                        className="pizza-block__image"
                        src= {imageUrl}
                        alt= 'Pizza'
                    />
                </Link>

                    <h4 className="pizza-block__title">{title}</h4>

                    <div className="pizza-block__selector">
                        <ul>
                            {
                                types.map((item : number,index : number) => (
                                    <li
                                        key={index}
                                        className={activeType === index ? 'active' : ''}
                                        onClick={() => setActiveType(index)}>
                                        {typeNames[item]}
                                    </li>
                                ))
                            }
                        </ul>
                        <ul>
                            {sizes.map((item : number,index : number) => (
                                <li
                                    key={index}
                                    onClick={() => setSizeActive(index)}
                                    className={index === sizeActive ? 'active' : ''}
                                >
                                    {item} см.
                                </li>
                            ))}
                        </ul>
                    </div>
                    <div className="pizza-block__bottom">
                        <div className="pizza-block__price">від {sizeActive > 0  ? price + 100 : price} грн. </div>
                        <button onClick={onClickAdd} className="button button--outline button--add ">
                                <svg
                                    width="12"
                                    height="12"
                                    viewBox="0 0 12 12"
                                    fill="none"
                                    xmlns="http://www.w3.org/2000/svg"
                                >
                                <path
                                    d="M10.8 4.8H7.2V1.2C7.2 0.5373 6.6627 0 6 0C5.3373 0 4.8 0.5373 4.8 1.2V4.8H1.2C0.5373 4.8 0 5.3373 0 6C0 6.6627 0.5373 7.2 1.2 7.2H4.8V10.8C4.8 11.4627 5.3373 12 6 12C6.6627 12 7.2 11.4627 7.2 10.8V7.2H10.8C11.4627 7.2 12 6.6627 12 6C12 5.3373 11.4627 4.8 10.8 4.8Z"
                                    fill="white"
                                />
                               </svg>
                            <span>Додати</span>
                            {addedCount > 0 && <i>{addedCount}</i>}
                        </button>
                    </div>
                </div>

            );
}

